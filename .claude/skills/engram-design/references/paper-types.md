...
claude-engram is one project, so this skill does not carry SAK-style subject categories
...
- Appendix carries the raw `engram eval` output and the reproducibility block.
...
- Method describes the architecture, the POEAA roles, and the contracts, grounded at HEAD
  ...
  Use `/engram-poeaa` to keep the pattern description accurate.
...
**What it is:** the integrative synthesis over the whole system — the claude-engram
...
- Discussion integrates memory research honestly (including where claude-engram diverges
...
and every number in `engram eval`; it is longer, not looser.
...
belongs in `engram-plan`; a code-impact analysis belongs in `engram-analyse`.
