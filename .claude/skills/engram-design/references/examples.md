...
Concrete claude-engram examples to reach for when a blank page is hard to face. All numbers
are drawn from [DESIGN.md](../../../../DESIGN.md)'s benchmark table and are illustrative —
a real paper reproduces them from `engram eval` this session and cites the commit.
...
budgets a naive design conflates. This paper specifies claude-engram, a local-first memory
...
**Source at HEAD**
- Recall path: [`plugins/engram/core/recall/__init__.py`](../../plugins/engram/core/recall/__init__.py)
- Capture service: [`plugins/engram/core/service.py`](../../plugins/engram/core/service.py)
- Store (Repository): [`plugins/engram/core/store.py`](../../plugins/engram/core/store.py)
- Benchmark harness: [`plugins/engram/bench/run_eval.py`](../../plugins/engram/bench/run_eval.py)
...
So basically claude-engram is a really powerful memory system that leverages cutting-edge
...
memory that "remembers everything" is not an option. This paper specifies claude-engram, a
