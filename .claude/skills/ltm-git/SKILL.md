---
name: ltm-git
description: GitHub and version-control workflows for the claude-ltm plugin repo — conventional commits (scoped to ltm/core/viewer/bench/docs/ci), branch + PR creation with pre-push ruff and unittest checks, and PR merge with an optional merge report. GitHub-only; no Linear, Slack, or release automation. Use when committing, branching, pushing, creating PRs, or merging.
...
Git and GitHub workflows for the **claude-ltm** repo (`lawrenceeldridge/claude-ltm`).
...
2. **Determine scope** — map changed paths to a scope (full table in
   ...
   | Changed path | Scope |
   |---|---|
   | `plugins/ltm/**` (general) | `ltm` *(default)* |
   | `plugins/ltm/core/**` (core-only, worth distinguishing) | `core` |
   | `plugins/ltm/viewer/**` | `viewer` |
   | `plugins/ltm/bench/**` | `bench` |
   | `README.md`, `DESIGN.md`, `.claude/**`, `CLAUDE.md` | `docs` |
   | CI config | `ci` |

   Prefer `ltm` unless a narrower scope adds real signal. Don't invent deep sub-scopes.
...
**Repo:** `lawrenceeldridge/claude-ltm` — a Python plugin (see [README.md](../../../README.md)).
Only `plugins/ltm/**` ships to installers; `.claude/` + `CLAUDE.md` are dev-only.
...
|---|---|---|
| Plugin (default) | `plugins/ltm/` | `ltm` |
| Pure-Python core | `plugins/ltm/core/` | `core` |
| Localhost viewer | `plugins/ltm/viewer/` | `viewer` |
| Recall benchmark | `plugins/ltm/bench/` | `bench` |
...
**User says:** `/ltm-git commit`

**Result:** Runs `git status`, sees changes under `plugins/ltm/core/recall.py`, picks
