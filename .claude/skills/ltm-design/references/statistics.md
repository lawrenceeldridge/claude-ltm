...
claude-ltm's sample sizes, and how to run and read the `ltm eval` benchmark that
...
A **measured claim** is a number you produced this session from `ltm eval` (or read
verbatim from source at HEAD) and can reproduce. An **asserted claim** is anything else.
Asserted quantitative claims do not belong in a claude-ltm paper. If you cannot run the
...
# Single backend
python3 bin/ltm eval --backends hash
```
...
> `python3 bin/ltm eval --backends "hash,fastembed"` from `plugins/ltm/` at commit
...
- [ ] Raw `ltm eval` output is in an appendix.
