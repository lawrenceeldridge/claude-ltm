...
This reference explains each of the six sections of a claude-ltm design paper in depth,
with the systems-research mapping and worked examples grounded in the plugin's own
evidence ([DESIGN.md](../../../../DESIGN.md), the `ltm eval` benchmark, the POEAA map).

The template is an empirical research paper. claude-ltm is a systems project, so the
...
> distinct budgets that a naive design conflates. This paper specifies claude-ltm, a
...
1. **Opening / context.** The problem in the world. For claude-ltm: an agent's context
...
2. **Related work.** What is already known and built. Synthesise, do not list. Three
   strands are load-bearing for claude-ltm:
...
3. **The gap.** What prior approaches miss. For claude-ltm: always-on retrieval tools
...
5. **Hypotheses / claims.** Formal and testable, stated as a numbered list. Each must be
   answerable by the Results section. Examples for claude-ltm:
...
Who/what was studied. For claude-ltm: the evaluation corpus (the labelled dataset —
...
Then the instrument: the `ltm eval` harness (`bench/run_eval.py`), and precise
...
vector-store practice and the memory-research grounding? Where claude-ltm diverges from
...
- **Source at HEAD** — every module the paper cites, with a repo-relative path
  (`plugins/ltm/core/recall/__init__.py`). A reader can check each claim against the
  named file.
...
- **Raw `ltm eval` output** — the verbatim benchmark run behind § 3, with the command
  and the commit hash.
