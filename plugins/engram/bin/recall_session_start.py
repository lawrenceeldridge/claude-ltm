...
MEMORY_FIRST_POLICY = (
    "[engram] Memory/index-first — at the start of a task, and before ANY codebase search or "
    "whole-file read, consult claude-engram first (measured ~2/3 fewer tokens than grep+read on "
    ...
    try:
        ...
        if cfg.embedding != "hash":
            ...
            if is_provisioned(cfg.data_dir):
                ...
                subprocess.Popen(
                    [sys.executable, str(ROOT / "bin" / "engram"), "setup"],
