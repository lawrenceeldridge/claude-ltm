...
via ENGRAM_TEST_NATS_URL. Skipped by the default stdlib suite (no nats-py, no server).

To run:
  docker run -d --rm -p 4223:4222 nats:latest --jetstream
  ENGRAM_TEST_NATS_URL=nats://localhost:4223 <python-with-nats-py> -m unittest tests.test_nats_bus
"""
...
_URL = os.environ.get("ENGRAM_TEST_NATS_URL")
_REASON = "requires nats-py + a live NATS at ENGRAM_TEST_NATS_URL"
...
class NatsBusIntegrationTests(unittest.TestCase):
    def setUp(self):
        ...
        self.stream = "ENGRAM_TEST_" + os.urandom(4).hex()  # unique per test
        os.environ["ENGRAM_DATA_DIR"] = self.tmp.name
        os.environ["ENGRAM_NATS_URL"] = _URL
        os.environ["ENGRAM_NATS_STREAM"] = self.stream
    ...
    def tearDown(self):
        ...
        try:
            ...
            pass
        self.bus.close()
        for k in ("ENGRAM_DATA_DIR", "ENGRAM_NATS_URL", "ENGRAM_NATS_STREAM"):
    ...
    def test_get_bus_returns_nats_when_up(self):
        ...
        try:
            ...
            os.environ.pop("ENGRAM_BUS", None)
