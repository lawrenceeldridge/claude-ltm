#!/usr/bin/env python3
"""Live paired A/B: tokens per task with the engram plugin on vs off.
...
- **off** — ``--settings bench/ab_off_settings.json`` (``engram@claude-engram``
  disabled), verified by scanning the off-arm session transcript for any engram
...
ENGRAM_MARKERS = ("mcp__plugin_engram", "plugins/engram/bin/")
...
def transcript_is_clean(session_id: str, project_path: str) -> bool | None:
    """True when the off-arm transcript shows no engram *activity*; None if not found.
    ...
    ignored — answer content about this repo legitimately mentions engram paths, and
    ...
    try:
        with path.open(encoding="utf-8", errors="replace") as fh:
            for line in fh:
                if not any(marker in line for marker in ENGRAM_MARKERS):
                    continue
                try:
