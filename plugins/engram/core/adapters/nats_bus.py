...
JetStream layout: one work-queue stream over ``engram.>``; a per-stage durable pull
consumer filters ``engram.*.<stage>``; publish sets ``Nats-Msg-Id`` = the WorkItem's
...
class NatsBus(MemoryBus):
    ...
    async def _connect(self) -> None:
        ...
        try:
            ...
            # already owns the `engram.>` subjects (a real misconfig). Confirm ours is there;
    ...
    def publish(self, item: WorkItem) -> None:
        self._ensure()
        subject = f"engram.{item.project_key}.{item.stage}"
