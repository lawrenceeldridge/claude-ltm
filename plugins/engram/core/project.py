...
An explicit ``.engram-root`` sentinel file takes precedence over the marker walk: drop
...
The nearest ``.engram-root`` ancestor wins.
"""
...
def resolve_project(cwd: str | None, markers: tuple[str, ...]) -> Project:
    ...
    # 1. An explicit .engram-root sentinel pins the project root (highest precedence).
