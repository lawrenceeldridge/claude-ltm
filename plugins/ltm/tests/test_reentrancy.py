...
Claude session must not fire ltm's hooks and capture the distiller prompt as a session
(the loop that stuffed the rescue queue with prompt-template payloads). Verifies:
  - `LTM_DISABLE` gates the shared `hooks_disabled()` helper,
...
class HooksDisabledHelperTests(unittest.TestCase):
    def setUp(self):
        self._saved = os.environ.get("LTM_DISABLE")
        os.environ.pop("LTM_DISABLE", None)

    def tearDown(self):
        if self._saved is None:
            os.environ.pop("LTM_DISABLE", None)
        else:
            os.environ["LTM_DISABLE"] = self._saved

    def test_off_by_default(self):
        self.assertFalse(hooks_disabled())

    def test_on_when_set(self):
        os.environ["LTM_DISABLE"] = "1"
        self.assertTrue(hooks_disabled())


class HookNoOpTests(unittest.TestCase):
    """Every hook exits 0 and does nothing when LTM_DISABLE=1 (driven as a subprocess)."""

    def test_all_hooks_noop_when_disabled(self):
        env = {**os.environ, "LTM_DISABLE": "1", "LTM_DATA_DIR": tempfile.mkdtemp()}
...
class DistillerEnvTests(unittest.TestCase):
    def test_claude_distiller_sets_disable_env(self):
        ...
        self.assertEqual(captured["env"].get("LTM_DISABLE"), "1")
