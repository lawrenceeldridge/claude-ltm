...
def _provision_with_uv(uv: str, venv: Path, py: str, log) -> None:
    log(f"[ltm] provisioning embedding env via uv (python {_UV_PYTHON})…")
...
def _provision_with_pip(venv: Path, py: str, log) -> None:
    ...
    log(f"[ltm] provisioning embedding env with {base} + pip (one-time, ~1 min)…")
...
def provision(data_dir: str | os.PathLike, log=print) -> bool:
    ...
    if is_provisioned(data_dir):
        log("[ltm] embedding env already provisioned")
        return True
    lock = data_dir / ".provision.lock"
    try:
        os.close(os.open(str(lock), os.O_CREAT | os.O_EXCL | os.O_WRONLY))
    except FileExistsError:
        log("[ltm] provisioning already in progress")
        return False
    try:
        ...
        log("[ltm] embedding env ready — semantic recall active next session")
        return True
    except (OSError, subprocess.SubprocessError, RuntimeError) as exc:
        log(f"[ltm] provisioning failed: {exc}")
        return False
    finally:
        try:
            lock.unlink()
